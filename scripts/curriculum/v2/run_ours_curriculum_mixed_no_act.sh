#!/bin/bash
# Ours: Mixed Disturbance (No Action Delay) (Residual Memory, WITH Curriculum)
# Config: Obs Delay=3, Bias=0.05, Dropout=hard
...
export RUN_NAME="ours_curriculum_mixed_no_act"

# 3. Disturbance Config (Max values for Curriculum)
# Stages: none -> easy -> med -> hard
# Hard stage will reach these values:
export OBS_DROPOUT_LEVEL="hard" 
export OBS_DELAY_K="3"
export ACTION_DELAY_K="0"
export OBS_BIAS_B="0.05"
...
echo "Disturbance Curriculum: Mixed (ObsDelay 3 + Bias 0.05 + Dropout Hard), NO ActDelay"
